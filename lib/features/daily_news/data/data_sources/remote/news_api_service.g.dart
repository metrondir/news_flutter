...
class _NewsApiService implements NewsApiService {
  ...
  if (_result.data != null && _result.data!['articles'] != null) {
    List<ArticleModel> value = _result.data!['articles']
        .where((dynamic articleData) =>
            articleData['urlToImage'] != null && articleData['urlToImage'].isNotEmpty)
        .map<ArticleModel>((dynamic i) =>
            ArticleModel.fromJson(i as Map<String, dynamic>))
        .toList();
    final httpResponse = HttpResponse(value, _result);
    return httpResponse;
  } else {
    // Handle the case where the URL is null or articles are not present.
